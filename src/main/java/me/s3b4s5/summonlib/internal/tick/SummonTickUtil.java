...
public final class SummonTickUtil {

    private SummonTickUtil() {}
    ...
    public static @Nullable NpcSummonCtx getNpcSummonCtxOrNull(
        ...
        final SummonDefinition def = resolveDefOrNull(base.tag);
        if (def == null) return null;
    ...
    private static @Nullable SummonDefinition resolveDefOrNull(SummonTag tag) {
        final SummonDefinition def = SummonRegistry.get(tag.getSummonId());
        return (def == null || def.tuning == null) ? null : def;
    }
    ...
    private static void dbgRemove(@Nullable Logger logger, boolean DEBUG, @Nullable UUID summonUuid, SummonTag tag, String msg) {
        ...
        logger.dbgSummons(true, null, summonUuid, tag.getSummonId(), msg);
    }
}
