...
public final class WormSummonSpawner {
    ...
    private static void attachTags(
        ...
        holder.putComponent(SummonLib.summonTagType(), new SummonTag(ownerUuid, summonId, slotCost, spawnSeq, variantIndex));

        WormTag wt = new WormTag();
        wt.chainId = chainId;
        wt.segmentIndex = segmentIndex;
        wt.spacing = spacingToPrev;

        holder.putComponent(SummonLib.wormTagType(), wt);
    }
    ...
    private static ExistingWorm findExistingWorm(Store<EntityStore> store, UUID ownerUuid, String summonId) {
        Query<EntityStore> q = Query.and(
                SummonLib.summonTagType(),
                SummonLib.wormTagType(),
        ...
        store.forEachChunk(q, (chunk, ccb) -> {
            for (int i = 0; i < chunk.size(); i++) {
                SummonTag st = chunk.getComponent(i, SummonLib.summonTagType());
                WormTag wt = chunk.getComponent(i, SummonLib.wormTagType());
